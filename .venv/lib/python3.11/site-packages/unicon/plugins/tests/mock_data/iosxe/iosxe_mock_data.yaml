...
general_enable:
  ...
  commands: &gen_enable_cmds
    ...
    "config check syntax":
      ...
      new_state: syntax_check
    "acm configlet create my_config":
      response:
        - |
          advanced configuration mode started.
          enter configuration commands, one per line.  end with cntl/z.
      new_state: acm_configlet
    "acm rules":
      response:
        - |
          Enter configuration rules mode, End with CNTL/Z.
      new_state: acm_rules
    "long_hostname":
...
general_config:
  ...
  commands: &general_config_cmds
    ...
    "username admin privilege 15 password Secret12345": "% Error: Password/key validation failed against default policy"
    "iox": ""
    ...
    "banner login *":
      new_state: banner_config_state
    "nat scale":
      new_state: general_config_nat_scale_confirm
      response: |
        Enable AOT and clear NAT Flow entries before enabling scale
    "end":
      new_state: general_enable

general_config_nat_scale_confirm:
...
standby_exec:
  ...
  commands:
    ...
    "cisco": ""
    "config term":
      new_state: configure_standby

configure_standby:
  prompt: "%N-standby(conf)#"
  commands:
    "end":
      new_state: standby_exec
...
acm_configlet_banner_input_percent:
  prompt: ""
  commands:
    "Updated - Unauthorized access to this device is strictly prohibited.": ""
    "Please contact corpnet@fb.com with any access requests.": ""
...
acm_configlet:
  ...
  commands:
    "interface loopback 1":
      new_state: acm_if
    "banner login %":
      response: "Enter TEXT message. End with the character '%'."
      new_state: acm_configlet_banner_input_percent
    "interface loopback 2":
      new_state: acm_if
...
acm_rules:
  ...
  commands:
    "match mode mdt-subscription-mode command no":
      response: "Match Rule exists with given submode: mdt-subscription-mode and cli_sring:no"
      new_state: rules_match

rules_match:
  prompt: "%N(rules-match)#"
  commands:
    "match mode interface command no ip address 10.43.188.58 255.255.255.192":
      response: "Match Rule exists with given submode: interface and cli_sring:no ip address 10.43.188.58 255.255.255.192"
    "action pre-apply":
      new_state: rules_match_action

rules_match_action:
  prompt: "%N(rules-match-action)#"
  commands:
    "end":
      new_state: general_enable
...
connect_ssh_refuse:
  preface: |
    Received disconnect from 127.0.0.1 port 22:2:  Requested line is busy!
...
general_enable_reload_to_rommon:
  prompt: "%N#"
  commands:
    <<: *gen_enable_cmds
    "reload":
...
general_enable_no_operating_mode:
  prompt: "%N#"
  commands:
    <<: *gen_enable_cmds
...
security_log_message:
  prompt: " SECURITY WARNING - Module: SSH, Command: crypto key generate rsa ..., Reason: SSH RSA host key uses insufficient key length, Remediation: Configure SSH RSA host key with minimum key length of 3072 bits"
  commands:
    "":
      new_state: general_exec

RSA_key_message:
  prompt: " %RSA key size needs to be atleast 3072 bitsfor SSH server functionality to be enabled"
  commands:
    "":
      new_state: general_exec

insecure_log_message:
  prompt: " INSECURE DYNAMIC WARNING - Module: SSH, Command: crypto key generate rsa modulus <modulus-size> label <label-name>, Reason: An SSH hostkey has been provisioned on the device with insufficient key length, Remediation: Please provision an SSH RSA hostkey with minimum modulus size of 3072 bits for enhanced security, Submode: exec, Parent CLI: Not Applicable"
  commands:
    "":
      new_state: general_exec
enable_reload_insecure_log1:
  prompt: "%N#"
  commands:
    <<: *gen_enable_cmds
    "reload":
      new_state: enable_reload_insecure_log2

enable_reload_insecure_log2:
  preface: "Press RETURN to get started!"
  prompt: "%N>"
  commands:
    "":
      new_state: insecure_log_message


enable_reload_RSA_log1:
  prompt: "%N#"
  commands:
    <<: *gen_enable_cmds
    "reload":
      new_state: enable_reload_RSA_log2

enable_reload_RSA_log2:
  preface: "Press RETURN to get started!"
  prompt: "%N>"
  commands:
    "":
      new_state: RSA_key_message

enable_reload_security_log1:
  prompt: "%N#"
  commands:
    <<: *gen_enable_cmds
    "reload":
      new_state: enable_reload_security_log2

enable_reload_security_log2:
  preface: "Press RETURN to get started!"
  prompt: "%N>"
  commands:
    "":
      new_state: security_log_message

general_login1:
  prompt: "Username: "
  commands:
    "notenable":
      new_state: general_password1

general_password1:
  prompt: "Password: "
  commands:
    "cisco":
      new_state: general_exec1
...
c8kv_controller_mode_enable:
  prompt: "%N#"
  commands:
    "enable": ""
    "controller-mode disable":
      new_state: c8kv_controller_mode_confirm

c8kv_controller_mode_confirm:
  prompt: "Continue? [confirm]"
  commands:
    "":
      new_state: c8kv_controller_mode_abort

c8kv_controller_mode_abort:
  prompt: "Do you want to abort? (yes/[no]):"
  commands:
    "no":
      new_state: c8kv_controller_mode_initial_config

c8kv_controller_mode_initial_config:
  prompt: "Would you like to enter the initial configuration dialog? [yes/no]: "
  commands:
    "no":
      new_state: c8kv_controller_mode_disable

c8kv_controller_mode_disable:
  prompt: "router>"
  commands:
    "enable":
      new_state: c8kv_controller_mode_autonomous_enable

c8kv_controller_mode_autonomous_enable:
  prompt: "router#"
  commands:
    "disable":
      new_state: c8kv_controller_mode_disable
