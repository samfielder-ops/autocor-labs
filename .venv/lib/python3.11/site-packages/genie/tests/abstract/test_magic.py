...
class Test_Lookup(unittest.TestCase):
    ...
    def setUpClass(cls):
        global test_lib
        global merge_base_lib
        global merge_ext_lib
        global Lookup

        from genie.abstract.magic import Lookup
        from genie.abstract import package
        from genie.tests.abstract import test_lib, merge_base_lib, merge_ext_lib
        cls.package_module = package

    def setUp(self):
        self.package_module._seen_set = set()
    ...
    def test_lookup_init(self):
        ...
        self.assertIs(l.packages['test_lib'],
    ...
    def test_lookup_pass_in_package_tuple(self):
        l = Lookup('nxos', 'cli', packages=(merge_base_lib, merge_ext_lib))

        self.assertEqual(l.simple.Feature.source, 'ext-nxos-cli')
        self.assertEqual(
            Lookup('nxos', 'yang', packages=(merge_base_lib, merge_ext_lib)).simple.Feature.source,
            'base-nxos-yang',
        )

    def test_lookup_dict_packages_unchanged(self):
        l = Lookup('nxos', 'cli', packages={'base': merge_base_lib,
                                            'ext': merge_ext_lib})

        self.assertEqual(l.base.simple.Feature.source, 'base-nxos-cli')
        self.assertEqual(l.ext.simple.Feature.source, 'ext-nxos-cli')
    ...
    def test_lookup_device_tuple_packages(self):
        class Device():
            one = 'nxos'
            two = 'cli'
            custom = {'abstraction': {'order': ['one', 'two']}}

        l = Lookup.from_device(Device(), packages=(merge_base_lib, merge_ext_lib))
