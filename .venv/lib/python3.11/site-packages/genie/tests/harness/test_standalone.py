...
class TestStandalone(unittest.TestCase):
    ...
    def test_retry_trigger_on_passed(self):
        """
        To test that a trigger with section_results=["passed"] and retries=2
        is retried even when it passes, to gauge test stability.
        ...
        |   |-- connect                                                           PASSED
        |   `-- configure                                                        SKIPPED
        |-- TriggerRetryOnPassed.uut                                              PASSED
        |   `-- test                                                              PASSED
        |-- TriggerRetryOnPassed.uut [Retry 1]                                    PASSED
        |   `-- test                                                              PASSED
        |-- TriggerRetryOnPassed.uut [Retry 2]                                    PASSED
        |   `-- test                                                              PASSED
        `-- common_cleanup                                                        PASSED
        Done!
        """
        ...
        try:
            log_contents = cmd(
                'pyats run job script/standalone_retry_trigger_script_job.py '
                '--testbed-file script/testbed.yaml '
                '--trigger-datafile script/standalone_retry_on_passed_trigger_datafile.yaml '
                '--crft-disable --btrace-disable --disable-device-info '
                '--replay script/mock_device --no-mail --no-upload --speed 2 '
                '--trigger-uids TriggerRetryOnPassed',
                cwd=cwd)
        except Exception as e:
            log_contents = e.output
        ...
        # TriggerRetryOnPassed.uut                                              PASSED
        p1 = re.compile(
            r'TriggerRetryOnPassed.uut\s+PASSED')
        # TriggerRetryOnPassed.uut [Retry 1]                                    PASSED
        p2 = re.compile(
            r'TriggerRetryOnPassed.uut \[Retry 1\]\s+PASSED')
        # TriggerRetryOnPassed.uut [Retry 2]                                    PASSED
        p3 = re.compile(
            r'TriggerRetryOnPassed.uut \[Retry 2\]\s+PASSED')
        ...
        self.assertTrue(check1, "Expected 'TriggerRetryOnPassed.uut PASSED' in output")
        self.assertTrue(check2, "Expected 'TriggerRetryOnPassed.uut [Retry 1] PASSED' in output")
        self.assertTrue(check3, "Expected 'TriggerRetryOnPassed.uut [Retry 2] PASSED' in output")
