...
class TestApi(unittest.TestCase):
    ...
    def test_api_deprecated_warning_identifies_original_api(self):
        self.device.api.function_data = AbstractTree.from_json({
            'touch_file': {
                'folders': {
                    'iosxe': {
                        'package': 'genie.tests.conf.base',
        ...
        with self.assertWarnsRegex(
                FutureWarning,
                (r'genie\.tests\.conf\.base\.test_api\.touch_file '
                 r'is deprecated\. please use create_empty_file\(\)')):
            result = self.device.api.touch_file(
                'bootflash:/', 'packages.conf', overwrite=True)

        self.assertEqual(
            result,
            (self.device, 'bootflash:/', 'packages.conf', True))
...
class TestCleanAPI(unittest.TestCase):
    ...
    def test_call_clean(self, clean_template, clean_template_default_args):
        ...
        expected_data = {"order": ["connect2"], "connect": {}, "connect2": {}}
