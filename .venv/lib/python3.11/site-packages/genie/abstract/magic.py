...
from .package import merge_packages
...
def get_caller_stack_pkgs(stacklvl = 1):
    ...
    # get the caller frame directly (faster than inspect.stack())
    frame = sys._getframe(stacklvl)
...
class Lookup(object):
    ...
    ---------
        ...
        packages (tuple): imported abstract packages to merge into a single
                          lookup namespace before resolving attributes.
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if isinstance(packages, tuple):
            # Tuple input means "build one merged abstract package" rather than
            # "return a wrapper of independently named packages".
    ...
    def __init__(self,
                 *tokens,
                 ...
                 packages=None,
                 device=None):
        """Create a lookup against one package or one merged package tree."""


        # extract abstract package object from given python package, and learn
        # the abstract tokens
        if packages is not None:
            # Tuple-based packages are merged into one synthetic package so the
            # caller can resolve them through a single lookup path.
    ...
    def from_device(cls,
        ...
        ---------
            ...
            packages (dict|tuple): Either a dictionary of package names to
                                   module objects, or a tuple of imported
                                   packages that should be merged into one
                                   lookup namespace.
    ...
    def tokens_from_device(
        cls, device: 'Device',
        token_attrs: list[str], package=None) -> dict[str, str]:
        ...
        package : str | AbstractPackage | module, optional
            Package context used to gather package-specific abstraction
            overrides and default token values, by default None.
        ...
        if package:
            ...
            recursive_update_extend(
                tb_abstraction_info, tb_abstraction_info.get(package_name, {}))
        ...
        if package:
            ...
            if isinstance(package, str):
                package_name = package
            recursive_update_extend(
                abstraction_info, abstraction_info.get(package_name, {}))
        ...
        # Apply package defaults after device/testbed-derived tokens so any
        # unspecified attributes still inherit the package's configured values.
        if package:
            if isinstance(package, str):
                pkg = importlib.import_module(package)
                abs_pkg = getattr(pkg, '__abstract_pkg', None)
            else:
                abs_pkg = getattr(package, '__abstract_pkg', None)
                if abs_pkg is None and hasattr(package, 'default_token_values'):
                    abs_pkg = package
            if abs_pkg:
